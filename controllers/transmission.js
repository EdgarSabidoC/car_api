const { Transmission } = require("../models"); // Referencia a lo exportado en models/index.js
...
/**
 * La función getItem busca y devuelve un elemento de la base de datos según el valor del parámetro transmissionIdOrType.
 * @param {*} req El objeto de solicitud HTTP.
 * @param {*} res El objeto de respuesta HTTP.
...
const getItem = async (req, res) => {
	let transaction;
	try {
		const { transmissionIdOrType } = req.params;

		// Se obtiene una instancia de la transacción:
		transaction = await sequelize.transaction();

		// Se ejecuta la consulta dentro de la transacción:
		const data = await Transmission.findOne({
			where: isNaN(transmissionIdOrType)
				? { type: transmissionIdOrType }
				: { id: transmissionIdOrType },
			transaction,
		});
...
/**
 * Obtiene una lista de elementos de la tabla transmission utilizando paginación y una transacción de base de datos.
...
const createItem = async (req, res) => {
	...
	try {
		...
		// Se ejecuta la consulta dentro de la transacción:
		const data = await Transmission.create(body, { transaction });
...
/**
 * Obtiene un elemento de la tabla transmission según su ID o código de producto, utilizando una transacción de base de datos.
 * @param {Object} req - Objeto de solicitud HTTP.
 * @param {Object} res - Objeto de respuesta HTTP.
