...
const { Price } = require("../models"); // Referencia a lo exportado en models/index.js
...
/**
 * La función getItem busca y devuelve un elemento de la base de datos según el valor del parámetro priceIdOrConcept.
 * @param {*} req El objeto de solicitud HTTP.
 * @param {*} res El objeto de respuesta HTTP.
...
const getItem = async (req, res) => {
	let transaction;
	try {
		const { priceIdOrConcept } = req.params;
		...
		const data = await Price.findOne({
			where: isNaN(priceIdOrConcept)
				? { concept: priceIdOrConcept }
				: { id: priceIdOrConcept },
			transaction,
		});
...
/**
 * Obtiene una lista de elementos de la tabla price utilizando paginación y una transacción de base de datos.
...
const createItem = async (req, res) => {
	...
	try {
		...
		// Se ejecuta la consulta dentro de la transacción:
		const data = await Price.create(body, { transaction });
...
/**
 * Obtiene un elemento de la tabla price según su ID o código de producto, utilizando una transacción de base de datos.
 * @param {Object} req - Objeto de solicitud HTTP.
 * @param {Object} res - Objeto de respuesta HTTP.
