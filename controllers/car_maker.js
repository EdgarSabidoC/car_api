const { CarMaker } = require("../models"); // Referencia a lo exportado en models/index.js
...
const getItems = async (req, res) => {
	...
	try {
		...
		const data = await CarMaker.findAll({
			transaction,
			order: [["id", "ASC"]],
		});
...
/**
 * La función getItem busca y devuelve un elemento de la base de datos según el valor del parámetro carMakerIdOrName.
 * @param {*} req El objeto de solicitud HTTP.
 * @param {*} res El objeto de respuesta HTTP.
...
const getItem = async (req, res) => {
	let transaction;
	try {
		const { carMakerIdOrName } = req.params;

		// Se obtiene una instancia de la transacción:
		transaction = await sequelize.transaction();

		// Se ejecuta la consulta dentro de la transacción:
		const data = await CarMaker.findOne({
			where: isNaN(carMakerIdOrName)
				? { name: carMakerIdOrName }
				: { id: carMakerIdOrName },
			transaction,
		});
...
/**
 * Obtiene una lista de elementos de la tabla maker utilizando paginación y una transacción de base de datos.
...
const createItem = async (req, res) => {
	...
	try {
		...
		// Se ejecuta la consulta dentro de la transacción:
		const data = await CarMaker.create(body, { transaction });
...
/**
 * Obtiene un elemento de la tabla maker según su ID o código de producto, utilizando una transacción de base de datos.
 * @param {Object} req - Objeto de solicitud HTTP.
 * @param {Object} res - Objeto de respuesta HTTP.
