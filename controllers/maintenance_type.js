const { MaintenanceType } = require("../models"); // Referencia a lo exportado en models/index.js
...
/**
 * La función getItem busca y devuelve un elemento de la base de datos según el valor del parámetro maintenanceTypeIdOrConcept.
 * @param {*} req El objeto de solicitud HTTP.
 * @param {*} res El objeto de respuesta HTTP.
...
const getItem = async (req, res) => {
	let transaction;
	try {
		const { maintenanceTypeIdOrConcept } = req.params;
		...
		const data = await MaintenanceType.findOne({
			where: isNaN(maintenanceTypeIdOrConcept)
				? { concept: maintenanceTypeIdOrConcept }
				: { id: maintenanceTypeIdOrConcept },
			transaction,
		});
...
/**
 * Obtiene una lista de elementos de la tabla maintenance_type utilizando paginación y una transacción de base de datos.
...
const createItem = async (req, res) => {
	...
	try {
		...
		// Se ejecuta la consulta dentro de la transacción:
		const data = await MaintenanceType.create(body, { transaction });
...
/**
 * Obtiene un elemento de la tabla maintenance_type según su ID o código de producto, utilizando una transacción de base de datos.
 * @param {Object} req - Objeto de solicitud HTTP.
 * @param {Object} res - Objeto de respuesta HTTP.
