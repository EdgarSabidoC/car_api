const { CarCondition } = require("../models"); // Referencia a lo exportado en models/index.js
...
const getItems = async (req, res) => {
	...
	try {
		...
		const data = await CarCondition.findAll({
			transaction,
			order: [["id", "ASC"]],
		});
...
/**
 * La función getItem busca y devuelve un elemento de la base de datos según el valor del parámetro conditionIdOrType.
 * @param {*} req El objeto de solicitud HTTP.
 * @param {*} res El objeto de respuesta HTTP.
...
const getItem = async (req, res) => {
	let transaction;
	try {
		const { conditionIdOrType } = req.params;

		// Se obtiene una instancia de la transacción:
		transaction = await sequelize.transaction();

		// Se ejecuta la consulta dentro de la transacción:
		const data = await CarCondition.findOne({
			where: isNaN(conditionIdOrType)
				? { type: conditionIdOrType }
				: { id: conditionIdOrType },
			transaction,
		});
...
/**
 * Obtiene una lista de elementos de la tabla car_condition utilizando paginación y una transacción de base de datos.
...
const createItem = async (req, res) => {
	...
	try {
		...
		// Se ejecuta la consulta dentro de la transacción:
		const data = await CarCondition.create(body, { transaction });
...
/**
 * Obtiene un elemento de la tabla car_condition según su ID o código de producto, utilizando una transacción de base de datos.
 * @param {Object} req - Objeto de solicitud HTTP.
 * @param {Object} res - Objeto de respuesta HTTP.
