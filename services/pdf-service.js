...
/**
 * Genera un PDF específico con encabezado y pie de página y lo pasa al siguiente middleware.
 * @param {Object} req - Objeto de solicitud HTTP.
 * @param {Object} res - Objeto de respuesta HTTP.
 * @param {Function} next - Función para pasar al siguiente middleware.
 * @throws {Error} Error al crear un PDF.
...
const createPdf = async (req, res, next) => {
	try {
		// Se debe de buscar información en la BD sobre el dealership con transacciones.
		const customer = "Juan Rodriguez Cárdenas";
		const dealership_name = "Donut-Motors Cars Emporium";
		const email = "example@email.com";
		const appointment_date = "19-05-2023";
		const appointment_time = "12:00";
		const car = "1G1YY3D70G5100001";
		const telephone = "9996381234";
...
module.exports = { buildPdf, createPdf };
